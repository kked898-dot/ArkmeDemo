    ...
    <AppShell
      mainPane={
        <div className="relative flex min-h-0 flex-1 flex-col h-full">
          ...
          />
          {recognizeToast && (
            <ArrangementRecognizeToast
              result={recognizeToast.result}
              originalMessage={recognizeToast.originalMessage}
              onConfirm={() => {
              ...
              onDismiss={() => setRecognizeToast(null)}
            />
          )}
        </div>
      }
...
function SettingsScreen({
  ...
  return (
    <div className="relative flex h-full flex-col bg-bg">
      ...
      {showAISettings && <AISettingsPage onBack={() => setShowAISettings(false)} />}
    </div>
  );
}
